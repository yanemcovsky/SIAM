...
class ResNet_Cifar(nn.Module):
    ...
    def __init__(self, block, layers, width=1, num_classes=10, input_size=32, weight_noise=False, act_noise_a=False,
        ...
        self.logits = torch.tensor([2 ** (-x) for x in range(num_classes)], requires_grad=False)
    ...
    def forward(self, x, add_noise=True):
        # print("self.noise_sd")
        # print(self.noise_sd)
        # print("self.noise_sd.grad")
        # print(self.noise_sd.grad)
        # print("if self.train and add_noise:")
        if add_noise:
            # x = x + torch.randn_like(x) * torch.abs(self.noise_sd)
            x = x + torch.randn_like(x) * self.noise_sd

            # print("x = x + torch.randn_like(x) * self.noise_sd")
    ...
    def predict(self, x, output, maxk):
        ...
        return self.monte_carlo_logits_predict(x, maxk, pred)

    def monte_carlo_logits_predict(self, x, maxk, pred):
        self.eval()
        with torch.no_grad():
            if self.m_test == 1:
            ...
            histogram = torch.zeros(x.shape[0], self.num_classes).to(x)

            for i in trange(self.m_test):
                # x_sample = x + torch.randn_like(x) * self.noise_sd
                # output = self.forward(x_sample, add_noise=False)
                output = self.forward(x)
                _, sorted_output = output.sort(dim=1, descending=True)
                # print("sorted_output")
                # print(sorted_output)
                # print("sorted_output.shape")
                # print(sorted_output.shape)
                # pred_i = sorted_output[:maxk]
                # _, pred_i_tmp = output.topk(maxk, 1, True, True)
                # if pred_i != pred_i_tmp:
                #     print("pred_i != pred_i_tmp")
                #     print("pred_i")
                #     print(pred_i)
                #     print("pred_i_tmp")
                #     print(pred_i_tmp)
                ...
                # sorted_output_flat = sorted_output.view(-1, 1)
                # # print("histogram")
                # # print(histogram)
                ...
                # print("sorted_output.shape")
                # print(sorted_output.shape)
                # # print("sorted_output_flat")
                # # print(sorted_output_flat)
                # print("sorted_output_flat.shape")
                # print(sorted_output_flat.shape)
                histogram_values = histogram.gather(1, sorted_output)
                histogram = histogram.scatter(1, sorted_output, histogram_values + self.logits)
                # print("histogram")
                # print(histogram)
                # print("histogram.shape")
                # print(histogram.shape)
            ...
            for j in range(maxk):
                predict[:, j] = torch.argmax(histogram, dim=1)
    ...
    def to(self, *args, **kwargs):
        ...
        self.logits = self.logits.to(*args, **kwargs).detach().requires_grad_(False)
        return self
