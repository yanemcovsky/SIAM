...
def get_args():
    ...
    parser.add_argument('--trans', type=str, metavar='PATH',
                        help='path to latest checkpoint (default: none)')
    ...
    parser.add_argument('--smootht', dest='smootht', action='store_true', help='Use weight noise')
    parser.add_argument('--tepgd', dest='tepgd', action='store_true', help='Use weight noise')
...
def optimize_hyper(args):
    ...
    if args.cpni:
        add_args = {'weight_noise': args.weight_noise, 'act_noise_a': args.act_noise_a,
    ...
    for iterations_idx, iterations in enumerate(args.iterations_list):
        ...
        if args.opt == 'sgd':
            optimizer = torch.optim.SGD(model_noise_parameter, args.learning_rate, momentum=args.momentum,
                                        weight_decay=args.decay, nesterov=args.nesterov_momentum)
        elif args.opt == 'adam':
            optimizer = torch.optim.Adam(model_noise_parameter, args.learning_rate, weight_decay=args.decay)
        else:
        ...
        for epoch in trange(args.start_epoch, args.epochs + 1):
            adv_train(model, train_loader, 0, optimizer, criterion, device, dtype, args.batch_size, args.log_interval,
                      args.attack, args.eps, 0.5, normalize)

            test_loss, accuracy1, accuracy5, test_loss_a, accuracy1_a, accuracy5_a = attack(model, val_loader,
                                                                                            criterion, att_object,
                                                                                            args.eps, normalize, device,
                                                                                            dtype)
...
def check_hyper(args):
    ...
    for iterations_idx, iterations in enumerate(args.iterations_list):
        for noise in args.noise_list:
            ...
            model, modelt, criterion = model.to(device=device, dtype=dtype), modelt.to(device=device,
                                                                                       dtype=dtype), criterion.to(
            ...
            if args.trans:
                if os.path.isfile(args.trans):
                    ...
                    optimizer.load_state_dict(checkpoint['optimizer'])
                    print("=> loaded checkpoint '{}' (epoch {})".format(args.trans, checkpoint['epoch']))
            ...
            if args.gen_adv:
                ...
                if args.attack == Transfer:
                    ...
                    a['width'] = args.width
                    if args.tepgd:
                        att_object = args.attack(model, criterion, modelt, EPGD_FGSMk)
                    else:
...
def main():
    ...
    if args.optimize_hyper_params:
        max_accuracy_iteration, max_accuracy_noise, max_adv_accuracy, max_accuracy = optimize_hyper(args)
        noise_sd = max_accuracy_noise
        m_test = max_accuracy_iteration
